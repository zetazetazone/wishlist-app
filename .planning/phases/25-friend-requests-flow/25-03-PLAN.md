...
must_haves:
  ...
  key_links:
    ...
    - from: "app/(app)/member/[id].tsx"
      to: "supabase friend_requests table"
      via: "direct query for request ID (getIncomingRequestId helper)"
      pattern: "from.*friend_requests.*eq.*from_user_id.*eq.*to_user_id.*eq.*pending.*single"
---
...
// Helper: Direct query for request ID (efficient - single row lookup instead of fetching all requests)
...
import { supabase } from '../../../lib/supabase';  // For direct request ID query
```
