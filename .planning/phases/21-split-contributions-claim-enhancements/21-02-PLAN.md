...
must_haves:
  ...
  artifacts:
    - path: "lib/contributions.ts"
      ...
      exports: ["openSplit", "pledgeContribution", "closeSplit", "getSplitStatus", "getClaimSummary"]
  key_links:
    - from: "lib/contributions.ts"
      to: "supabase.rpc"
      via: "RPC function calls"
      pattern: "supabase\\.rpc\\('open_split|pledge_contribution|close_split|get_split_status'"
---
...
Output: lib/contributions.ts with 5+ exported functions and supporting types.
</objective>
...
5. `getContributors(itemId: string): Promise<Contributor[]>`
...
6. `getClaimSummary(celebrantUserId: string, groupId: string): Promise<ClaimSummary>`
...
Verify all 6 functions exported and properly typed.
  </verify>
  <done>
lib/contributions.ts exports openSplit, pledgeContribution, closeSplit, getSplitStatus, getContributors, getClaimSummary with proper types and error handling.
  </done>
</task>
...
- New RPC function signatures (open_split, pledge_contribution, close_split, get_split_status)
...
Check that lib/contributions.ts compiles without `@ts-ignore` comments.
  ...
  <done>
Database types regenerated with new RPC function signatures and updated gift_claims/wishlist_items column types.
  </done>
</task>
...
<verification>
- [ ] lib/contributions.ts exists with all 6 functions exported
