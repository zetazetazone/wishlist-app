...
<task type="auto">
  <name>Task 3a: Create notification inbox screen</name>
  <files>
    app/(app)/(tabs)/notifications.tsx
    app/(app)/(tabs)/_layout.tsx
  </files>
...
<task type="auto">
  <name>Task 3b: Create profile viewing screen</name>
  <files>
    app/profile/[id].tsx
  </files>
  <action>
...
4. Display:
   ...
   - Display name
   - Birthday (formatted nicely, e.g., "March 15" using date-fns format)
   - Member since date
...
Run `ls app/profile/` to verify profile screen exists.
  ...
  <done>
Profile screen displays member information with avatar, name, birthday, and member since date.
  </done>
</task>
...
- Push notification infrastructure (from Plan 01)
  ...
  <how-to-verify>
**Test A: Onboarding WITHOUT photo (minimum required fields)**
...
4. Complete onboarding:
   ...
   - Do NOT add profile photo
   - Tap Continue
5. Verify you land on main app (wishlist tab)
6. Check profile shows placeholder avatar (no photo)

**Test B: Onboarding WITH photo upload (PROF-02)**
1. Clear app data or use different account
2. Log in and go through onboarding
3. Complete onboarding:
   - Enter display name
   - Select birthday
   - Tap avatar area to upload photo
   - Select image from gallery
   - Verify image preview shows in avatar circle
   - Tap Continue
4. Navigate to your profile
5. Verify uploaded photo is displayed (not placeholder)

**Test C: Notification Inbox and Profile Viewing**
1. Navigate to Notifications tab - should show empty state
2. Navigate to a group and tap on a member to view their profile
3. Verify profile shows name, birthday, avatar (if set)
