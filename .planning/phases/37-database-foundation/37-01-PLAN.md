...
must_haves:
  truths:
    ...
    - "RLS policies support both group_id (legacy) and wishlist_id (new) access patterns"
  artifacts:
  ...
  key_links:
    ...
    - from: "gift_claims RLS"
      ...
      pattern: "wi.group_id"
    - from: "wishlist_items RLS SELECT"
      to: "group_id OR wishlist_id"
      via: "Dual-access pattern"
      pattern: "(group_id.*is_group_member|wishlist_id.*wishlists)"
---
...
**PART 2: Partial unique index for one default per user (WISH-04 enforcement)**
```sql
CREATE UNIQUE INDEX idx_wishlists_user_default
  ON public.wishlists(user_id)
  WHERE is_default = TRUE;
```
This index enforces WISH-04: exactly one default wishlist per user at the database level.
...
**PART 7: Updated RLS policy for wishlist_items SELECT (dual-access pattern)**
...
This preserves legacy group_id access while adding new wishlist_id access.
...
4. Verify WISH-04 enforcement: attempt `INSERT INTO wishlists (user_id, name, is_default) VALUES ('some-user-id', 'Second Default', TRUE)` for user with existing default - should fail with unique constraint violation
  </verify>
  <done>
- wishlists table exists with all columns
- Partial unique index on is_default=TRUE enforces one default per user (WISH-04)
...
- Trigger creates default wishlist for new users (on public.users INSERT)
  </done>
</task>
...
**Validation 2: No user has multiple default wishlists (WISH-04 enforcement)**
```sql
...
HAVING COUNT(*) > 1;
-- Expected: 0 rows
-- The partial unique index idx_wishlists_user_default prevents this at write time,
-- but this validates no existing data violates the constraint.
```
...
Test query to verify celebrant cannot see their own claims:
```sql
-- This query shows what a celebrant would see for claims on their own items
-- It should return EMPTY for claims on items belonging to the celebrant
SELECT gc.id
FROM public.gift_claims gc
JOIN public.wishlist_items wi ON gc.wishlist_item_id = wi.id
WHERE wi.user_id = auth.uid();
-- Expected: empty result set when run as the celebrant (birthday person)
-- The gift_claims RLS policy uses wi.group_id for access, NOT wishlist_id,
-- and excludes the item owner via "AND wi.user_id != auth.uid()"
```
Document in comments that gift_claims SELECT policy still uses wi.group_id for group membership check AND excludes item owner. The existing celebrant exclusion logic remains valid because:
1. It checks group membership via wi.group_id (unchanged)
2. It excludes items where wi.user_id = auth.uid() (unchanged)
3. Adding wishlist_id to wishlist_items doesn't alter these checks

**Validation 6: Partial unique index prevents duplicate defaults**
```sql
...
-- Attempting a duplicate default would fail (tested in Task 1 verify)
```
...
3. Verify celebrant exclusion: The gift_claims SELECT policy joins through wishlist_items.group_id (not wishlist_id) and excludes wi.user_id = auth.uid(), so existing claims visibility is unchanged
4. Verify WISH-04: Run `SELECT indexname FROM pg_indexes WHERE tablename = 'wishlists' AND indexname = 'idx_wishlists_user_default';` returns 1 row
  </verify>
  <done>
- All validation queries pass (0 orphans, 0 duplicate defaults, all items linked)
- WISH-04 enforcement validated (partial unique index exists and prevents duplicates)
- Celebrant exclusion confirmed preserved (gift_claims RLS unchanged, test query documented)
...
2. **RLS verification:**
   - Verify dual-access pattern in wishlist_items SELECT policy (supports both group_id and wishlist_id)
   - Verify cannot delete default wishlist via RLS
   - Verify gift_claims RLS unchanged (celebrant exclusion intact via wi.group_id and wi.user_id check)

3. **WISH-04 verification:**
   - Verify partial unique index exists: `SELECT indexname FROM pg_indexes WHERE tablename = 'wishlists' AND indexname = 'idx_wishlists_user_default';`
   - Verify no duplicate defaults: `SELECT user_id, COUNT(*) FROM wishlists WHERE is_default = TRUE GROUP BY user_id HAVING COUNT(*) > 1;` returns 0 rows
...
- [ ] Partial unique index idx_wishlists_user_default exists (WISH-04 enforcement)
...
- [ ] Dual-access RLS on wishlist_items supports both group_id and wishlist_id access patterns
- [ ] Trigger creates default wishlist for new users
- [ ] Celebrant exclusion test query returns empty for celebrant viewing own claims
</success_criteria>
