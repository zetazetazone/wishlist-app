...
must_haves:
  truths:
    ...
    - "Personal details are globally accessible across groups (same data viewed from any group context)"
  artifacts:
...
- `grep 'getPersonalDetails' app/(app)/member/[id].tsx` confirms service function is imported and used for data fetching
- `grep 'router.push.*personal-details' app/(app)/settings/profile.tsx` confirms navigation wiring
  </verify>
  <done>
...
- PROF-06 (global sharing): Verify getPersonalDetails(memberId) returns same data regardless of which group context navigation originated from. Personal details are stored per-user (not per-group) and RLS allows any authenticated user to read, enabling cross-group visibility.
</verification>
