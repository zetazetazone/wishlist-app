...
<task type="auto">
  ...
  <files>lib/storage.ts, package.json</files>
  <action>
**FIRST: Install required dependency**
Run: `npx expo install expo-image-manipulator`

This installs the Expo-compatible version of expo-image-manipulator for image compression.

**THEN: Extend lib/storage.ts** with two new exported functions:
...
Keep existing uploadAvatar() and getAvatarUrl() unchanged.
  ...
  <verify>
Run: `grep "expo-image-manipulator" package.json`
...
Confirm compression import and usage.
  ...
  <done>expo-image-manipulator installed, lib/storage.ts exports uploadGroupPhoto() with compression and getGroupPhotoUrl()</done>
</task>
...
1. Dependency check:
   - `grep "expo-image-manipulator" package.json` shows the package is installed
...
2. "Upload service handles image compression and format validation"
   -> Task 2: expo-image-manipulator installed, uploadGroupPhoto() compresses to 800px width, 0.8 quality
