...
<objective>
Complete translation file coverage by auditing codebase and adding ~114 missing translation keys

Purpose: Close TRANS-04 and TRANS-05 gaps - translation files have 286 keys but ~400 are required. The shortfall includes Alert messages, placeholders, and section titles found throughout the app.
...
# Source files for translation audit
@src/i18n/locales/en.json
@src/i18n/locales/es.json
</context>
...
<task type="auto">
  <name>Task 1: Audit codebase for missing translation keys</name>
  <files>src/i18n/locales/en.json</files>
  <action>
Systematically audit the app codebase to identify missing translation keys:

1. **Alert.alert messages** - Found 100+ Alert.alert calls with hardcoded strings
   Locations: app/(app)/, app/auth/, app/group/, app/(onboarding)/
   Need keys for: 'Success', 'Error', 'Required', 'Confirm', 'Warning', and message bodies

2. **Placeholder strings** - Found 14 placeholder attributes
   Need keys for: "Enter your name", "Search by name or email", "Enter group name", etc.

3. **Section titles** - Found in group settings and other screens
   Need keys for: "Group Info", "Group Mode", "Budget", "Members", "Invite Code", "Danger Zone"

4. **Action strings** - Found throughout app
   Need keys for: "Coming Soon", "Transfer Admin", "Remove Member", "Leave Group", "Delete Group"

Add the following namespaces and keys to en.json:
...
Count final keys to verify ~400 target reached.
  </action>
  <verify>
...
- All Alert.alert strings have translation keys
- All placeholder strings have translation keys
- All section titles have translation keys
- JSON is valid
...
<task type="auto">
  <name>Task 2: Create matching Spanish translations</name>
  <files>src/i18n/locales/es.json</files>
  <action>
Add corresponding Spanish translations for all new keys added to en.json.
...
<task type="auto">
  <name>Task 3: Verify key count reaches ~400 target</name>
...
If count is below 380, identify additional missing strings and add them.
...
2. All Alert.alert messages have corresponding translation keys
3. All placeholder strings have corresponding translation keys
4. All section titles have corresponding translation keys
