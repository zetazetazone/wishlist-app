...
Create the item detail screen with full-bleed hero image and item information display.

## Success Criteria

- [ ] Route `/wishlist/[id]` renders ItemDetailScreen
- [ ] Hero image displays full-bleed at ~45% screen height
- [ ] expo-image with blur placeholder and caching
- [ ] Item info: title, brand (parsed), price, description
- [ ] Priority stars displayed
- [ ] "Go to Store" button opens URL
- [ ] Transparent header with back button
- [ ] Loading skeleton and error states
- [ ] Special item types render correctly (surprise_me, mystery_box)
...
### Task 3: Implement Hero Image Section

Create the full-bleed hero with special item handling:
    ...
    <View style={[styles.heroPlaceholder, { backgroundColor: placeholder.backgroundColor }]}>
      ...
      <Text style={styles.placeholderText}>
        {item.item_type === 'surprise_me'
          ? t('wishlist.itemTypes.surpriseMe')
          : t('wishlist.itemTypes.mysteryBox')}
      </Text>
    </View>
  );
};
...
### Task 4: Implement Item Info Section

Display title, brand, price, description:
...
### Task 5: Implement Header and Layout
...
### Task 7: Add getWishlistItem to lib

If not existing, add to `lib/wishlistItems.ts`:
...
- [ ] Navigate to `/wishlist/[valid-id]` - screen loads
- [ ] Hero image displays at ~45% screen height
- [ ] Blur placeholder shows during image load
- [ ] Title, price, priority stars display correctly
- [ ] Brand parsed and displayed if present in title
- [ ] "Go to Store" opens external URL
- [ ] Back button navigates back
- [ ] Options button visible (no action yet)
- [ ] Loading spinner shows during data fetch
- [ ] Error state shows for invalid item ID
- [ ] Surprise Me item shows question mark icon
- [ ] Mystery Box item shows gift icon with tier
- [ ] TypeScript compiles without errors

## Dependencies

- expo-image (installed in Phase 33)
- expo-router (existing)
- lib/wishlistItems.ts (may need getWishlistItem function)
- utils/wishlist.ts (formatItemPrice, getImagePlaceholder, parseBrandFromTitle)

## Risks

- **Linear gradient**: React Native doesn't support CSS gradients. May need `expo-linear-gradient` or use absolute positioned View with backgroundColor and opacity.

## Notes

- Claim UI section is placeholder - implemented in 35-02
- Options sheet action is placeholder - implemented in Phase 36
- Navigation wiring is done in 35-03
